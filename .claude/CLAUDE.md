...
- **No module outside `usage_monitor_for_claude/platforms/` may import `ctypes.windll`, `winreg`, `msvcrt`, `fcntl`, `gi`, or use `subprocess.CREATE_NO_WINDOW`.** Everything OS-specific lives behind the platform layer; the rest of the package must import cleanly on both systems
...
- `setup_console()` (Windows) must leave a standard stream the caller already redirected alone. `CONOUT$` names the console device, so opening it bypasses `> log.txt` and `-RedirectStandardOutput` alike and hands a bug reporter an empty file - the console is for a stream that has no redirection of its own, and nothing else. `GetStdHandle` needs an explicit `restype`, because ctypes' default `c_int` truncates a 64-bit HANDLE. The unit tests mock `ctypes` and therefore prove none of this: verify a change here against a real GUI-subsystem process (`pythonw.exe` behaves like the frozen build), never against the tests alone
...
- One process monitors exactly one Claude config directory; `api`, `settings` and `i18n` hold that directory in module-level state, and that is deliberate - it keeps the credential-handling code small enough to audit. Several accounts are therefore never handled in-process: `__main__` resolves the list (`--config-dir` repeated or `os.pathsep`-joined, else `CLAUDE_CONFIG_DIR` split the same way), and when it holds more than one directory it calls `launch.launch_account_instances()` and exits. Each child is the ordinary single-account app with `--config-dir=<own>`; a child whose directory is already monitored exits through the existing single-instance guard, so running the combined command twice is harmless
- `launch.spawn_app_instance()` is the only place that starts a copy of the app - the tray-menu restart goes through it as well. Frozen and source runs differ in the command line, and PyInstaller's `_PYI_`/`_MEI` variables must be dropped so the new process extracts its own bundle: keep both in that one function, never open-code a `Popen` of `sys.executable` elsewhere
- Every child receives the whole resolved set in `USAGE_MONITOR_CONFIG_DIRS` (`instance_id.LAUNCH_CONFIG_DIRS_ENV`), and its own directory in `CLAUDE_CONFIG_DIR` so the `claude update` it may run sees a single path. Two suffixes exist because two things are identified: `config_dir_suffix()` names the *instance* (mutex, lock file - always per directory), `autostart_suffix()` names the *launch* (registry value, `.desktop` file - one entry for the whole set, order-independent, so every child toggles the same entry and **Start at login** brings all of them back). For a single directory both suffixes are identical, which keeps entries written by earlier versions valid; a set that includes `~/.claude` still gets a suffix, or it would overwrite the legacy default entry
- `autostart_config_dir_argument()` is what the autostart command stores: the resolved set joined with `os.pathsep`, or `None` for the plain default directory. The platform modules only quote it - the shape of the value is decided once, in `instance_id`
- The separator is `os.pathsep`, exactly like `PATH`: `;` on Windows, `:` on Linux. Windows drive letters make `:` impossible there, and `;` is legal in POSIX paths, so no single character works on both. Do not add a second separator "for convenience" - the README documents one per platform and the tests pin `CONFIG_DIR_SEPARATOR == os.pathsep`
- The parent never waits for its children. With `--verbose` it prints one line per started instance and passes the flag on; a frozen child then allocates its own console, since the parent's is gone by the time it starts
...
- The authentication messages (`auth_expired`, `auth_expired_short`, `no_token`, `warn_login`) name the **CLI** and end with the command `claude auth login` - never a bare "log in to Claude Code". The app authenticates with the credentials file the CLI writes, and a Claude Code that is visibly running does not necessarily keep that file current. "Open Claude Code to log in again" therefore reads as already done, and the user goes looking for a bug in the monitor instead - issue #89 is a full report written from that misreading. Translations keep the command verbatim
...
- `or {}` guards a `null`, not a type change. A key that arrives as a list or a string is truthy and passes straight into the next `.get()`, raising `AttributeError`. Inside a `fetch_*` function its own `except` catches that; anywhere the result reaches `Cache.update()` unguarded it escapes `poll_loop()` and raises a crash box, so a shape change in one undocumented field stops all polling for the session. Not hypothetical: a reader of `spend.balance` chained `or {}` through two levels and would have ended the poll loop on a `spend` arriving as a list - it was removed before it shipped. Chain with `or {}` where the response shape is settled, and check `isinstance` where a new or undocumented block is being read
...
- `API_URL_PREPAID_CREDITS` is the one endpoint carrying a path parameter, and it stays the only one: the prepaid balance is addressed per organization, so the uuid cannot be known at build time. It is a single constant with one `{org_uuid}` placeholder, and `fetch_prepaid_credits()` matches the value against `_ORG_UUID_PATTERN` before substituting it, abandoning the request when it does not match - so the substituted segment can only ever be hex digits and dashes, never a slash, a dot-dot or a scheme. The pattern is anchored `\A...\Z`, not `^...$`: `$` also matches before a trailing newline, and `tests/test_api.py` covers exactly that case. A future endpoint needing a path parameter follows this shape - one constant, one placeholder, validated before substitution - or it does not ship
...
- Every PyInstaller one-file build is flagged by a handful of heuristic engines, and each release starts the wave again: seven issues so far (#46, #60, #76, #77, #79, #94, #95). The bundle unpacks itself into a temp directory and starts the interpreter from there, which is what a packer does and what the `!ml` models weight. Nothing in the code causes it and nothing in the code can fix it
- `README.md` owns the explanation in **Antivirus Warnings**, and it sits *above* the download link on purpose: a reader who meets the warning already knowing about it does not file an issue, while an explanation found afterwards only answers one. `.github/ISSUE_TEMPLATE/config.yml` lists it as the first contact link, so it appears in the template chooser before an issue can be opened (`blank_issues_enabled: false` makes that unavoidable). Answer a new report by linking that section, never by restating it - `tests/test_docs_links.py` guards the anchor, because GitHub resolves a renamed heading to nothing and the funnel would fail silently
- The SHA256 in the release notes carries a second job because of this: it is what turns "this is a false positive" from a promise into something the reporter can check. Keep it in every release for that reason as much as for the WinGet one
- Options already weighed and rejected, so they do not get re-proposed: buying a certificate (the app is free, and a SignPath Foundation application is pending), `--onedir` (it would help most, and it would break "single EXE, no installation" - the first feature bullet), and scripting the WDSI submission with a headless browser (it would put the Microsoft account credentials in CI secrets and fail silently when the form changes). What is left is a per-release submission to <https://www.microsoft.com/wdsi/filesubmission> as a "Software developer" dispute - the hash is fixed at build time, so it can be filed before publishing
