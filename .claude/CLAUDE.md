...
- A quota field is any dict entry with a non-null `utilization`; `extra_usage` has a separate structure and is handled independently. Whether that quota *applies to the account* is `is_active_quota()`: a reset window, a name `parse_field_name()` can read, or the `from_account_limits` marker `_merge_scoped_limits()` sets on the fields it synthesises. A bar additionally needs the `resets_at` key itself - that is what keeps the differently shaped `extra_usage` block out of `expand_popup_fields()`
- The name clause in `is_active_quota()` is load-bearing and must not be simplified away. The API lists quota types before they apply, under code names (`nimbus_quill`, `tangelo`), carrying a utilization of 0 and no reset window - structurally identical to a real quota sitting between a reset and its next use, so nothing but the name separates them. Drop the clause and `five_hour` disappears from the popup after every reset, until the next token is spent. It holds only while Anthropic keeps naming periods `<number word>_<unit>`: a `monthly` or `daily` field would not parse and would blink out in that window. Growing `_NUMBER_WORDS` does not help, because those are not number words - the marker covers it for model-scoped limits, and a top-level field with such a name is the trigger to revisit this
...
- Model-scoped limits (e.g. a weekly Fable limit) arrive only inside the `limits` array via `scope.model`, not as top-level fields - `_merge_scoped_limits()` in `api.py` normalizes each into a synthetic top-level field (e.g. `seven_day_fable`) so all of the above applies unchanged. The period prefix is derived from the same-`group` non-scoped limit's shared `resets_at` (never hardcoded); an existing top-level field is never overwritten, and inactive scoped limits (no `resets_at`) are still surfaced at 0% - the `from_account_limits` marker is what keeps those visible when the inherited prefix is not a name `parse_field_name()` can read
