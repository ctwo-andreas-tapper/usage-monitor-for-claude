...
- One process monitors every Claude config directory of the launch. `account.Account` (config dir + label) is threaded through every credential read: `api.*(account)`, `UsageCache(account)`, `claude_cli.refresh_token(account)` (runs `claude update` with `CLAUDE_CONFIG_DIR` set for that account). Nothing holds a "current" directory, and the app never writes `CLAUDE_CONFIG_DIR` into its own environment
- `app.AccountMonitor` owns everything per account: tray icon (`run_detached`, one pystray thread each), cache, poll loop, reset alignment, alert baselines, event commands. `app.UsageMonitorForClaude` is the shell: account list, one shared popup, quit/restart, theme, autostart, quick action. A click on any icon opens the same popup; `_polling_throttled()` reads `shell.popup_open` so an open popup holds every account at the normal cadence
- With more than one account, tooltips and toast titles carry `[label] `, and every event-command environment carries `USAGE_MONITOR_ACCOUNT` / `USAGE_MONITOR_ACCOUNT_EMAIL`
- The popup payload (`popup._popup_data`) groups by quota field over the union of all accounts' fields, one bar per reporting account, `account_index` on every bar; DOM keys are `field:index`. Single account renders as before (no badges, no group heading)
- `instance_id.launch_suffix()` names the mutex / lock file / autostart entry for the whole set (order-independent hash, empty for the single default dir so legacy entries stay valid). `resolve_config_dirs()` is the one place that reads `--config-dir` / `CLAUDE_CONFIG_DIR`; `__main__` and the platform helpers both go through it
- `launch.spawn_app_instance()` is still the only place that starts a copy of the app (tray-menu restart, passing the whole set as one `--config-dir`)
- One `usage-monitor-settings.json` for the whole app (exe-adjacent, then `~/.claude`); per-config-directory files are not read
- The separator is `os.pathsep`, exactly like `PATH`. Do not add a second separator
